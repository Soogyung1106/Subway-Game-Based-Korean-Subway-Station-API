...
//함수
void * handle_clnt1(void * arg); //read 한 후 send_mssg 함수(wirte하는 함수) 호출 -> 쓰레드 
void * handle_clnt2(void * arg);
void * handle_clnt3(void * arg);
...
int main(int argc, char *argv[])
{
	...
	{	
		...
		if(clnt_cnt<=2){
			...
			//>>>>>>>>>>>>>>>>>>>>>>>>>>>
			if(clnt_cnt == 0)
				pthread_create(&t_id, NULL, handle_clnt1, (void*)&clnt_sock); //handle_clnt 쓰레드
			if(clnt_cnt == 1)
				pthread_create(&t_id, NULL, handle_clnt2, (void*)&clnt_sock); //handle_clnt 쓰레드
			if(clnt_cnt == 2)
				pthread_create(&t_id, NULL, handle_clnt3, (void*)&clnt_sock); //handle_clnt 쓰레드
			//>>>>>>>>>>>>>>>>>>>>>>>>
...
void * handle_clnt1(void * arg)
{
	...
	while((str_len=read(clnt_sock, msg, sizeof(msg)))!=0){ //쓰레드가 끝나지 않도록 while문
		...
		msg_turn = "clnt1 차례입니다.\n"; // 배열에 참가자 이름 담기 
		send_msg(msg_turn, str_len); //누구 차례입니다.
		sleep(1);
		//>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>
